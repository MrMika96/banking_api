...
class ContactSerializer(serializers.ModelSerializer):
    ...
    contact_id = serializers.PrimaryKeyRelatedField(queryset=User.objects.all(), write_only=True, source='contact')

    class Meta:
        model = Contact
        fields = ['id', 'user', 'contact',
                  'contact_id', 'favorite', 'created_at']
...
class ContactBulkCreateSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        ...
        users = User.objects.filter(profile__phone__in=validated_data['phone_numbers']).values_list('id', flat=True)
        results = []
        for user in users:
